...
async def _check_card_async(card: str, proxy: dict | None = None):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_executor, _pp_lib.check_cc, card, proxy)
...
async def cmd_pp(msg: Message, command: CommandObject):
    ...
    t0 = time.time()
    try:
        status, response = await _check_card_async(card, proxy=proxy)
