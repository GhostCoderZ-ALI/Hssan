...
from commands.b3_5_lib import check_b3_5
from functions.proxy_gate import get_user_proxy
import database.db as db

router = Router()

@router.message(Command("b3-5"))
async def cmd_b35(message: types.Message):
    args = message.text.split(maxsplit=1) if message.text else []
    if len(args) < 2:
        await message.answer("Usage: <code>/b3-5 CC|MM|YY|CVV</code>", parse_mode='html')
        return
    ...
    progress = await message.answer("⏳ Checking Braintree $5...")
    try:
        is_live, msg = await asyncio.to_thread(check_b3_5, cc, mm, yy, cvv, proxy)
    ...
    try:
        await db.log_check(message.from_user.id, raw, "b3-5", "Braintree$5", "", "LIVE" if is_live else "DECLINED", msg, 0.0)
    except:
        pass
