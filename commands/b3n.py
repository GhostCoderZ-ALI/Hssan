...
from commands.b3n_lib import check_b3n
from functions.proxy_gate import get_user_proxy
import database.db as db

router = Router()

@router.message(Command("b3n"))
async def cmd_b3n(message: types.Message):
    args = message.text.split(maxsplit=1) if message.text else []
    if len(args) < 2:
        await message.answer("Usage: <code>/b3n CC|MM|YY|CVV</code>", parse_mode='html')
        return
    ...
    progress = await message.answer("⏳ Checking Braintree Auth...")
    try:
        is_live, msg = await asyncio.to_thread(check_b3n, cc, mm, yy, cvv, proxy)
    ...
    try:
        await db.log_check(message.from_user.id, raw, "b3n", "Braintree Auth", "", "LIVE" if is_live else "DECLINED", msg, 0.0)
    except:
        pass
