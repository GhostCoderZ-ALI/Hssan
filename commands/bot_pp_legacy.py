"""Library module — provides ``check_cc`` for the PayPal $1 gate.

Originally this file ran as a standalone telebot script with its own bot
token and command handlers. The active bot is the aiogram instance in
``main.py``; the only thing other modules import from here is ``check_cc``.
The standalone-bot scaffolding below is kept inert via a no-op stub so the
legacy decorator/handler bodies still parse without registering anything.
"""
import cloudscraper, base64, re, time, os, json, threading, hashlib, requests, random, datetime, queue, urllib3
...
class _InertBot:
    """Stub so legacy ``@bot.message_handler`` decorators are no-ops."""
