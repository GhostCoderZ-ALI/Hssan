"""Stripe gate via Dilaboards - /st single, /mst mass.
...
from functions.proxy_gate import get_user_proxy_dict
...
async def _check_async(card: str, proxy: dict | None = None):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_executor, _st_lib.check_cc, card, proxy)


@router.message(Command("st", prefix="/."))
async def cmd_st(msg: Message, command: CommandObject):
    ...
    if not ok:
        ...
        return
    ok_p, proxy, err_p = await get_user_proxy_dict(uid)
    if not ok_p:
    ...
    t0 = time.time()
    try:
        status, response, brand = await _check_async(card, proxy=proxy)
...
async def cmd_mst(msg: Message, command: CommandObject, bot: Bot):
    uid = msg.from_user.id
    if await db.is_banned(uid):
        await msg.answer(f"{EMOJI['ban']} You are banned.", parse_mode=ParseMode.HTML)
        return
...
async def _run_mass(msg: Message, command: CommandObject, bot: Bot, proxy: dict | None = None, prefilled_blob: str | None = None):
    """Shared mass-checker body for /mst."""
    uid = msg.from_user.id
