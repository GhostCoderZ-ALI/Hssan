...
from commands.st2_lib import check_st2
...
async def cmd_st2(message: types.Message):
    args = message.text.split(maxsplit=1) if message.text else []
    if len(args) < 2:
        await message.answer("Usage: <code>/st2 CC|MM|YY|CVV</code>", parse_mode='html')
        return
    ...
    try:
        await db.log_check(message.from_user.id, raw, "st2", "Stripe1337", "", "LIVE" if is_live else "DECLINED", msg, 0.0)
    except:
        pass
