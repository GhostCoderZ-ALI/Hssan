...
from commands.st15_lib import check_st15
from functions.proxy_gate import get_user_proxy
import database.db as db

router = Router()

@router.message(Command("st15"))
async def cmd_st15(message: types.Message):
    args = message.text.split(maxsplit=1) if message.text else []
    if len(args) < 2:
        await message.answer("Usage: <code>/st15 CC|MM|YY|CVV</code>", parse_mode='html')
        return
    ...
    progress = await message.answer("⏳ Checking Stripe $15...")
    try:
        is_live, msg = await asyncio.to_thread(check_st15, cc, mm, yy, cvv, "monthly", proxy)
    ...
    try:
        await db.log_check(message.from_user.id, raw, "st15", "Stripe$15", "", "LIVE" if is_live else "DECLINED", msg, 0.0)
    except:
        pass
