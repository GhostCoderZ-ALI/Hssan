"""Shopify gate — /sh single, /msh mass.

Powered by ``commands.lib_shop_api.check_card`` (direct Shopify API).
"""
import re
import time
...
@router.message(Command("sh", prefix="/."))
async def cmd_sh(msg: Message, command: CommandObject, bot: Bot):
    ...
    if not raw:
        await msg.answer(
            ...
            f"<code>/sh &lt;url&gt; cc|mm|yy|cvv</code>\n"
            f"<code>/sh cc|mm|yy|cvv</code> (uses default site)\n"
...
@router.message(Command("msh", prefix="/."))
async def cmd_msh(msg: Message, command: CommandObject, bot: Bot):
    uid = msg.from_user.id
    if await db.is_banned(uid):
        await msg.answer(f"{EMOJI['ban']} You are banned.", parse_mode=ParseMode.HTML)
        return
    ...
    for idx, card in enumerate(cards, 1):
        ...
        try:
            await progress.edit_text(
                f"{EMOJI['hitting']} <b>SH {idx}/{len(cards)}</b>\n"
