...
def extract_hcaptcha_sitekey(html: str) -> Optional[str]:
    """Try to find the hCaptcha sitekey in a Stripe checkout page."""
    patterns = [
        r'data-sitekey="([^"]+)"',
        r"data-sitekey='([^']+)'",
        r'sitekey\s*:\s*"([^"]+)"',
...
def solve_hcaptcha(site_key: str, page_url: str, api_key: str) -> Optional[str]:
...
def get_checkout_info_sync(url: str, proxy: str = None, max_retries: int = 2) -> Dict[str, Any]:
    ...
    try:
        ...
        # hCaptcha detection – automatically extract sitekey
        radar = init_data.get("radar_options", {})
        hc = radar.get("hcaptcha", {})
        if hc.get("enabled") or hc.get("site_key"):
            result["hcaptcha_enabled"] = True
            result["hcaptcha_sitekey"] = hc.get("site_key")
        else:
            result["hcaptcha_enabled"] = False
...
def charge_card_sync(
    ...
    """
    Charge a card using TLS client. Falls back to aiohttp on failure.
    Hard timeout prevents hanging threads.
    """
    ...
    def _run():
        for attempt in range(max_retries + 1):
            try:
                ...
                if hcaptcha_token:
                    conf_data["hcaptcha_token"] = hcaptcha_token
    ...
    # Run in a thread with a hard timeout
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            result["response"] = "Hard timeout (possible captcha)"
            result["time"] = round(time.perf_counter() - start, 2)
            return result
    ...
    if "error" in conf:
        ...
        if st == "succeeded":
            ...
            if action_type == "redirect_to_url":
                ...
                if any(kw in redirect_url.lower() for kw in [
                    "hcaptcha", "captcha", "turnstile", "challenge",
                ]):
                    result["status"] = "HCAPTCHA"
                    result["response"] = "hCaptcha – unsupported"
                else:
                    result["status"] = "3DS"
